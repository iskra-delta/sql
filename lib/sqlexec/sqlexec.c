...
static int validate_where(const sqlexec_program *program,
    const sql_where *where)
{
    ...
    if (where->root == (unsigned char)sql_where_nil || where->node_count == 0
        ...
        || where->node_count > program->where_node_count
        || where->value_count > program->where_value_count) {
        return 0;
    }
    ...
    while (depth > 0) {
        ...
        case sql_where_in:
            ...
            for (index = 0; index < node->value_count; index++) {
                if (!value_present(&program->where_values[
                    node->value_first + index])) {
                    return 0;
                }
            }
            break;
    ...
    unsigned char child_count)
{
    ...
    case sqlexec_run_subquery:
    case sqlexec_delete_temp:
        return child_count == 0 && name_present(node->data.named.name)
            ? 0 : -1;
    ...
    case sqlexec_create_table:
        ...
        return span_valid(program->column_count, node->data.table.columns)
            ? 0 : -1;
    case sqlexec_build_index:
    case sqlexec_register_index:
        if (child_count != 0
    ...
    case sqlexec_index_scan_eq:
        if (child_count != 0
            || !name_present(node->data.index_probe.index_name)) {
            return -1;
        }
        return value_present(&node->data.index_probe.value) ? 0 : -1;
    case sqlexec_index_scan_range:
        if (child_count != 0
            || !name_present(node->data.index_range.index_name)) {
            return -1;
        }
        if (node->data.index_range.lower_operator
            > sql_compare_greater_equal) {
            return -1;
        }
        if (node->data.index_range.upper_operator
            > sql_compare_greater_equal) {
            return -1;
        }
        if (node->data.index_range.lower_operator != sql_compare_invalid
            && !value_present(&node->data.index_range.lower_value)) {
            return -1;
        }
        if (node->data.index_range.upper_operator != sql_compare_invalid
            && !value_present(&node->data.index_range.upper_value)) {
            return -1;
        }
        return 0;
    case sqlexec_filter:
        return child_count == 1 && validate_where(program, &node->data.where)
            ? 0 : -1;
    case sqlexec_project:
        ...
        if (node->data.project.select_all) {
            return node->data.project.names.count == 0
                ...
                && node->data.project.qualifiers.count == 0 ? 0 : -1;
        }
        ...
        return 0;
    case sqlexec_make_record:
        return child_count == 0
            && span_valid(program->value_count, node->data.values)
            ? 0 : -1;
    case sqlexec_append_record:
    case sqlexec_count_rows:
        return child_count == 1 ? 0 : -1;
    case sqlexec_apply_assignments:
        return child_count == 1
            && span_valid(program->assignment_count,
    ...
    unsigned char ref)
{
    ...
    case sql_where_compare:
        ...
        dump_char(out, ' ');
        dump_value(out, &program->where_values[node->value_first]);
        break;
    case sql_where_in:
        dump_column_ref(out, node->qualifier, node->column_name);
        dump_text(out, " IN (");
        for (index = 0; index < node->value_count; index++) {
            ...
            dump_value(out, &program->where_values[
                node->value_first + index]);
        }
        dump_char(out, ')');
        break;
    ...
    case sql_where_or:
        dump_char(out, '(');
        dump_where_expression(out, program, where, node->left);
        dump_text(out, node->type == sql_where_and ? " AND " : " OR ");
        dump_where_expression(out, program, where, node->right);
        dump_char(out, ')');
        break;
    default:
        dump_text(out, "?");
    ...
    const sqlexec_node *node)
{
    ...
    case sqlexec_index_scan_eq:
        ...
        dump_text(out, " = ");
        dump_value(out, &node->data.index_probe.value);
        break;
    case sqlexec_index_scan_range:
        dump_char(out, ' ');
        dump_text(out, node->data.index_range.index_name);
        if (node->data.index_range.lower_operator != sql_compare_invalid) {
            dump_char(out, ' ');
            dump_text(out, compare_name(
                node->data.index_range.lower_operator));
            dump_char(out, ' ');
            dump_value(out, &node->data.index_range.lower_value);
        }
        if (node->data.index_range.upper_operator != sql_compare_invalid) {
            dump_text(out, " .. ");
            dump_text(out, compare_name(
                node->data.index_range.upper_operator));
            dump_char(out, ' ');
            dump_value(out, &node->data.index_range.upper_value);
        }
        break;
    case sqlexec_filter:
        dump_char(out, ' ');
        dump_where_expression(out, program, &node->data.where,
            node->data.where.root);
        break;
    case sqlexec_project:
        dump_char(out, ' ');
        if (node->data.project.select_all) {
    ...
    case sqlexec_make_record:
        ...
        dump_uint(out, node->data.values.count);
        break;
    case sqlexec_apply_assignments:
...
const char *sqlexec_opcode_name(sqlexec_opcode opcode)
{
    ...
    case sqlexec_show_databases: return "show_databases";
    ...
    case sqlexec_build_index: return "build_index";
    case sqlexec_register_index: return "register_index";
    case sqlexec_unregister_table_indexes: return "unregister_table_indexes";
    case sqlexec_unregister_database_indexes: return "unregister_database_indexes";
    case sqlexec_open_table: return "open_table";
    case sqlexec_close_table: return "close_table";
    case sqlexec_table_scan: return "table_scan";
    case sqlexec_join_scan: return "join_scan";
    case sqlexec_index_scan_eq: return "index_scan_eq";
    case sqlexec_index_scan_range: return "index_scan_range";
    case sqlexec_filter: return "filter";
    case sqlexec_project: return "project";
    case sqlexec_make_record: return "make_record";
    case sqlexec_append_record: return "append_record";
    case sqlexec_apply_assignments: return "apply_assignments";
    case sqlexec_write_current: return "write_current";
    case sqlexec_delete_current: return "delete_current";
    case sqlexec_count_rows: return "count_rows";
    case sqlexec_count_affected: return "count_affected";
    case sqlexec_emit_rows: return "emit_rows";
    case sqlexec_emit_count: return "emit_count";
...
{
    if (count == 0
        || (unsigned short)(program->column_count + count) > sql_max_columns) {
        return -1;
    }

    *first_out = program->column_count;
    memcpy(program->columns + program->column_count, columns,
        (unsigned short)(count * sizeof(sql_column)));
    program->column_count = (unsigned char)(program->column_count + count);
    return 0;
}
...
{
    if (count == 0
        || (unsigned short)(program->value_count + count) > sql_max_columns) {
        return -1;
    }

    *first_out = program->value_count;
    memcpy(program->values + program->value_count, values,
        (unsigned short)(count * sizeof(sql_value)));
    program->value_count = (unsigned char)(program->value_count + count);
    return 0;
}
...
int sqlexec_add_assignments(sqlexec_program *program,
    ...
    unsigned char *first_out)
{
    if (count == 0 || (unsigned short)(program->assignment_count + count)
        > sql_max_columns) {
        return -1;
    }

    *first_out = program->assignment_count;
    memcpy(program->assignments + program->assignment_count, assignments,
